...
# Borrowed from:
# https://github.com/nebula-plugins/nebula-release-plugin/blob/master/gradle/buildViaTravis.sh
...
#  ./gradlew -Prelease.travisci=true snapshot $SWITCHES
  ./gradlew build $SWITCHES             # Temporarily disable snapshot releases
elif [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "$TRAVIS_TAG" != "" ]; then
